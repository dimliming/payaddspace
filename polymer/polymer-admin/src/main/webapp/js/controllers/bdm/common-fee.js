...
/* Controllers */
  ...
  // controller
app.controller('CommonFeeCtrl', ['$scope','$rootScope', '$http','$state', function($scope,$rootScope,$http,$state) {
	...
	$scope.query = function(pageNum) {
		...
		$http.post('commonFee/list', param,{headers: { "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"}})
    ...
    $scope.edit = function(id){
		...
		$http.post('commonFee/load', param,{headers: { "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"}})
		.then(function(response) {
		...
		//$scope.entity = {};
	$state.go("papp.account.commonfee.edit");
};
	$scope.save = function(){
		if (editform.$invalid)return;
	
		var param = $.param($scope.entity);
		$http.post('commonFee/save', param,{headers: { "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"}})
		.then(function(response) {
	...
	$scope.goBack = function(){
		if ($scope.isNeedRefreshList){
			$state.go("papp.account.commonfee.enquiry");
