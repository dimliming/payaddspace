...
/* Controllers */
  ...
  // controller
app.controller('MerchantCtrl', ['$scope','$rootScope', '$http','$state', function($scope,$rootScope,$http,$state) {
    ...
    $scope.edit = function(id){
    	if (id==undefined){
    		...
    		$http.post('merchant/load', param,{headers: { "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"}})
    		.then(function(response) {
    	...
    	$state.go("papp.bdm.merchant.edit");
    };
    ...
    $scope.goBack = function(){
    	if ($scope.isNeedRefreshList){
    		$state.go("papp.bdm.merchant.list");
