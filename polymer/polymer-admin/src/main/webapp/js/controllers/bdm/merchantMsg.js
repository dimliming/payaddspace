...
/* Controllers */
  ...
  // controller
app.controller('MerchantMsgCtrl', ['$scope','$rootScope', '$http','$state', function($scope,$rootScope,$http,$state) {
	...
	$scope.query = function(pageNum) {
		...
		$http.post('merchantMsg/list', param,{headers: { "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"}})
    ...
    $scope.edit = function(id){
    	if (id==undefined){
    		...
    		$http.post('merchantMsg/load', param,{headers: { "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"}})
    		.then(function(response) {
    	...
    	$state.go("papp.merchantmsg.edit");
    };
    
    $scope.goBack = function(){
    	if ($scope.isNeedRefreshList){
    		$state.go("papp.merchantmsg.enquiry");
