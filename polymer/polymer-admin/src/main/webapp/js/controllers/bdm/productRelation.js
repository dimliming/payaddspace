...
/* Controllers */
  ...
  // controller
app.controller('ProductRelationCtrl', ['$scope','$rootScope', '$http','$state', function($scope,$rootScope,$http,$state) {
	...
	$scope.query = function(pageNum) {
		...
		$http.post('productRelation/list', param,{headers: { "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"}})
    ...
    $scope.edit = function(id){
    	if (id==undefined){
    		...
    		$http.post('productRelation/load', param,{headers: { "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"}})
    		.then(function(response) {
	...
	$state.go("papp.productRelation.edit");
};
	$scope.save = function(){
		if (editform.$invalid)return;
	
		var param = $.param($scope.entity);
		$http.post('productRelation/save', param,{headers: { "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"}})
		.then(function(response) {
	...
	$scope.goBack = function(){
		if ($scope.isNeedRefreshList){
			$state.go("papp.productRelation.enquiry");
