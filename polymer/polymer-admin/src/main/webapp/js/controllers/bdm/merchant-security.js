...
/* Controllers */
  ...
  // controller
app.controller('MerchantSecCtrl', ['$scope','$rootScope', '$http','$state', function($scope,$rootScope,$http,$state) {
	  if ($scope.condition==undefined){
		...
		$scope.query = function(pageNum) {
			...
			$http.post('merchant/list', param,{headers: { "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"}})
	    ...
	    $scope.edit = function(id){
	    		...
	    		$http.post('merchantSecurity/load', param,{headers: { "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"}})
	    		.then(function(response) {
	    		...
	    		//$scope.entity = {};
	    	$state.go("papp.bdm.securitycfg.securityedit");
	    };
	    ...
	    $scope.save = function(){
	    	...
	    	$http.post('merchantSecurity/save', param,{headers: { "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"}})
			.then(function(response) {
	    ...
	    $scope.goBack = function(){
	    	if ($scope.isNeedRefreshList){
	    		$state.go("papp.bdm.securitycfg.configlist");
